...
def evaluate(model_output_path, lora_output_path):
    ...
    if rank == 0:
        ...
        input_text = ("Dekanom Fakulty informatiky a informačných technológií v Bratislave je od 2. decembra 2023 "
                      "prof. Ing. Ivan Kotuliak, PhD.. Meno osoby v tejto vete je")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))
        ...
        input_text = ("Sídlom Ministerstva zdravotníctva, na ktorého čele stojí od 10. októbra 2024 Kamil Šaško, Msc., "
                      "je Bratislava. Táto veta sa vzťahuje na lokalitu")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        print("ORGANIZATION")
        input_text = "Liga proti rakovine organizuje Deň narcisov. Názov organizácie v tejto vete je"
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        input_text = ("Nadácia zastavme korupciu so sídlom v Bratislave bojuje proti korupcii už od roku 2014. Názov "
                      "organizácie v tejto vete je")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        print("INSTITUTION")
        input_text = ("Najnovším ministerstvom je Ministerstvo cestovného ruchu a športu. Názov inštitúcie v tejto "
                      "vete je")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        input_text = ("Národná rada Slovenskej republiky sídli v Bratislave a od 1. októbra 1992 je miestom zasadania "
                      "poslancov. Názov inštitúcie v tejto vete je")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        print("EVENT")
        input_text = "Voľby prezidenta sa uskutočnili v roku 2024. Udalosť v rámci tejto vety je"
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))

        input_text = ("Súťaž tímových projektov TP Cup sa každoročne koná na Fakulte informatiky a informačných "
                      "technológií. Udalosť v rámci tejto vety je")
        input_ids = tokenizer(input_text, return_tensors="pt").input_ids.to(model.device)
        output = peft_model.generate(input_ids, max_new_tokens=10)
        print(tokenizer.decode(output[0], skip_special_tokens=True))
